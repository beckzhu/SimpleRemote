    ...
    public partial class RemoteControl_ssh : BaseRemoteControl
    {
            ...
            :base(contentControl)
        {
            ...
            try
            {
                ...
                _putty.SetCallback(_event_error, _event_connected, _event_securityAlert, _event_verifyhostkey, _event_storehostkey, _event_keydown, _event_debug);
                if (!_putty.Init())
                {
        ...
        public override void Connect(DbItemRemoteLink linkSettings, FinalItemSetting finalItemSetting)
        {
            ...
            FinalItemSetting_ssh finalSetting = (FinalItemSetting_ssh)finalItemSetting;
            ...
            Task.Run(() =>
               {
                   ...
                   string pw = null;
                   if (string.IsNullOrEmpty(linkSettings.PrivateKey))
                   {
                       pw = string.IsNullOrEmpty(linkSettings.Password) ? "" : $"-pw {linkSettings.Password}";
                   }
                   else
                   {
                       _privateKey = System.IO.Path.GetTempPath() + linkSettings.Id;
        ...
        {
            byte[] value = Database.GetSshHostKey(key);
        ...
        private void StoreHostKey_Event(string key, IntPtr otherstr, int readlen)
        {
            ...
            Database.SetSshHostKey(key, value);
        }
        /// <summary>
        /// 键盘按下事件
        /// </summary>
